...
version  <- "mrwaterFullOutputs_USDm3"
year     <- "y2010"
scenario <- "ssp2"
...
currentCroparea          <- setYears(read.magpie(paste0(inputdatapath, "cropareaLUH", ".mz")), 
                                     year) 
currentCropland          <- dimSums(currentCroparea, dim = "crop")
currentCroplandIrrigated <- collapseNames(currentCropland[, , "irrigated"])
currentCroplandTotal     <- dimSums(currentCropland, dim = "irrigation")
cropareaShr              <- read.magpie(paste0(inputdatapath, "cropareaShr.mz"))
...
comAgWW      <- collapseNames(read.magpie(paste0(inputdatapath, "comAguses.mz"))[,,scenario][,,"currHumanWWtotal"]) / 1000
comAgWC      <- collapseNames(read.magpie(paste0(inputdatapath, "comAguses.mz"))[,,scenario][,,"currHumanWCtotal"]) / 1000
comAgWCoff   <- collapseNames(comAgWC[,,"off"])
comAgWCon    <- collapseNames(comAgWC[,,"on"])

# Non-agricultural uses (in mio. km^3)
watNonAgWW   <- collapseNames(read.magpie(paste0(inputdatapath, "nonAguses.mz"))[,,scenario][,,"currHumanWWtotal"]) / 1000
watNonAgWC   <- collapseNames(read.magpie(paste0(inputdatapath, "nonAguses.mz"))[,,scenario][,,"currHumanWCtotal"]) / 1000
```

```{r ReportCurrentArea, include = TRUE, echo = FALSE}
writeLines(paste0("Globally, an area of ",
                  round(dimSums(currentCroplandIrrigated, dim = 1)),
                  " Mha is reported to be irrigated according to LUH. \nOf these irrigated areas, ",
                  round(dimSums(irrigAreaCurrent[,,"off"], dim = 1)),
                  " Mha could be fulfilled \ngiven the local water availability in this study. \nIF EFR were to be maintained, it would be ",
                  round(dimSums(irrigAreaCurrent[,,"on"], dim = 1)), " Mha.")) 
```

```{r ReportCurrentVolume, include = TRUE, echo = FALSE}
writeLines(paste0("Globally, a consumptive water volume of ",
                  round(dimSums(watComAgWC, dim = 1)),
                  " km3/yr is required to irrigate \nthe given cropmix (FAO country-level production pattern of 2010) on currently irrigated areas to its full potential. \nOf these irrigation water requirements, ",
                  round(dimSums(comAgWCoff, dim = 1)),
                  " km3/yr could be fulfilled \ngiven the local water availability in this study. \nThis volume would be reduced to ",
                  round(dimSums(comAgWCon, dim = 1)),
                  " km3/yr if the EFR were to be maintained."))
```
...
print(paste0("Global non-ag. consumption (unsustainable): ",
             round(dimSums(watNonAgWC[,,"off"], dim = 1)),
             " km^3/yr"))
print(paste0("Global non-ag. consumption (sustainable): ",
             round(dimSums(watNonAgWC[,,"on"], dim = 1)),
             " km^3/yr"))
```
...
yieldgainwaterIRRunsus <- read.magpie(paste0(inputdatapath, "yieldgainwater_actUNSUS.mz"))
yieldgainwaterIRRsus   <- read.magpie(paste0(inputdatapath, "yieldgainwater_actSUS.mz"))
...
yieldgainwaterCURunsus <- read.magpie(paste0(inputdatapath, "yieldgainwater_curUNSUS.mz"))
yieldgainwaterCURsus   <- read.magpie(paste0(inputdatapath, "yieldgainwater_curSUS.mz"))
...
yieldgainwaterPOTunsus <- read.magpie(paste0(inputdatapath, "yieldgainwater_potUNSUS.mz"))
yieldgainwaterPOTsus   <- read.magpie(paste0(inputdatapath, "yieldgainwater_potSUS.mz"))
...
plotIADcurve <- function(version     = "HalfEarth",
  ...
  if (aggregation == "countries") {
    ...
    df <- data.frame(Region = df$Region,
                     ...
                     IrrigArea = df$Value,
                     stringsAsFactors = FALSE)
    df$currentCroplandIrrigated <- rep(as.array(dimSums(currentCroplandIrrigated, dim = c(1.1, 1.2))), length(df$Group) /
                             length(as.array(dimSums(currentCroplandIrrigated, dim = c(1.1, 1.2)))))
    df$LUHCropland  <- rep(as.array(dimSums(currentCroplandTotal, dim = c(1.1, 1.2))), length(df$Group) / 
                             length(as.array(dimSums(currentCroplandTotal, dim = c(1.1, 1.2)))))
    ...
    tmp <- framestructure
    for (b in selectedBasins) {
      tmp$Value[tmp$Region == b] <- dimSums(currentCroplandIrrigated[toolSelectRiverBasin(basinname = b), , ], dim = 1)
      tmp$Value1[tmp$Region == b] <- dimSums(currentCroplandTotal[toolSelectRiverBasin(basinname = b), , ], dim = 1)
    }
    df$currentCroplandIrrigated <- tmp$Value
  ...
  out <- ggplot(df, aes(x = IrrigArea, y = GT, color = Group, group = Group, shape = Group)) +
                ...
                geom_point(aes(x = currentCroplandIrrigated, y = 0), size = 5, color = "black", shape = 17) +
...
croplandcellshare <- calcOutput("CellAreaShare", 
                                x = currentCroplandTotal, 
                                aggregate = FALSE)

# share potentially irrigated of total cell area
irrigcellshare    <- calcOutput("CellAreaShare", 
                                x = avlArea, 
                                aggregate = FALSE)
```
...
```{r ReadInCurrent, include=FALSE, echo=FALSE}
# Input data
input <- "shrHumanUsesFulfilled"

# Read in data
x <- collapseNames(read.magpie(paste0(inputdatapath, input, ".mz"))[, year, "off.ssp2.committedAg"])

# Exclude very small cropland areas
x[croplandcellshare < 0.01] <- NA

legendtitle  <- "Fulfilled Share"
legendlimit  <- c(0, 1)
...
a <- calcOutput("CellAreaShare", x = a, 
                aggregate = FALSE)
b <- calcOutput("CellAreaShare", x = b, 
                aggregate = FALSE)
c <- calcOutput("CellAreaShare", x = c, 
                aggregate = FALSE)
...
a <- mrwaterplots::toolRasterTransform(x = a)
b <- mrwaterplots::toolRasterTransform(x = b)
c <- mrwaterplots::toolRasterTransform(x = c)

# Get land mask and country borders
tmp            <- mrwaterplots::toolPrepareLandMask()
landMask       <- tmp$landMask
worldCountries <- tmp$worldCountries

### Plot 3 graphs in one jpeg
...
x0Share    <- calcOutput("CellAreaShare", x = x0,
                aggregate = FALSE)
x500Share  <- calcOutput("CellAreaShare", x = x500,
                aggregate = FALSE)
x1000Share <- calcOutput("CellAreaShare", x = x1000,
                aggregate = FALSE)

x0Sharesus    <- calcOutput("CellAreaShare", x = x0sus,
                aggregate = FALSE)
x500Sharesus  <- calcOutput("CellAreaShare", x = x500sus,
                aggregate = FALSE)
x1000Sharesus <- calcOutput("CellAreaShare", x = x1000sus,
                aggregate = FALSE)
...
# Areas that would only be irrigated in lowest yield gain class (unsustainable)
...
# Areas that would only be irrigated in lowest yield gain class (sustainable)
...
# Transform magpie object to raster object
x0Share        <- mrwaterplots::toolRasterTransform(x0Share)
x500Share      <- mrwaterplots::toolRasterTransform(x500Share)
x1000Share     <- mrwaterplots::toolRasterTransform(x1000Share)
...
# Transform magpie object to raster object
x0Sharesus        <- mrwaterplots::toolRasterTransform(x0Sharesus)
x500Sharesus      <- mrwaterplots::toolRasterTransform(x500Sharesus)
x1000Sharesus     <- mrwaterplots::toolRasterTransform(x1000Sharesus)
...
print(paste0("Non-protected areas that are suitable for cropland activities: ",
             round(dimSums(potlandUnprotect, dim = 1), digits = 1)))
```
...
protectArea <-  calcOutput("CellAreaShare", x = protectArea, 
                aggregate = FALSE)
...
# Transform magpie object to raster object
protectArea <- mrwaterplots::toolRasterTransform(protectArea)
# correct rounding
protectArea[protectArea < 0] <- 0
protectArea[protectArea > 1] <- 1
...
yldGainCURrf <- dimSums(yieldGain * (collapseNames(IrrigAreaEconCURUNSUS[,,"0.off"])-currentCroplandIrrigated), dim = 1) /
                            dimSums((collapseNames(IrrigAreaEconCURUNSUS[,,"0.off"])-currentCroplandIrrigated), dim = 1)
...
tmpIrrig    <- dimSums(yieldGain * currentCroplandIrrigated, dim = 1) / dimSums(currentCroplandIrrigated, dim = 1)
tmpCropland <- dimSums(yieldGain * currentCroplandTotal, dim = 1) / dimSums(currentCroplandTotal, dim = 1)
tmpRainfed  <- dimSums(yieldGain * (currentCroplandTotal-currentCroplandIrrigated), dim = 1) / dimSums((currentCroplandTotal-currentCroplandIrrigated), dim = 1)
...
writeLines(paste0(
  "The average yield  gain on currently irrigated areas is ", round(tmpIrrig, digits = 1), "USD per hectare.", 
...
weights::wtd.hist(x = yieldGain_GLO, weight = yieldgainareaPOTunsus[,,"0"],
              breaks = c(0, 200, 400, 600, 800, 1000, 1200), col = "#fcbba1", ylim = c(0, 3000),
              main = "(a) Global average commodity prices\n", ylab = "Yield gain area (in Mha)",
              xlab = "")
weights::wtd.hist(x = yieldGain_ISO, w = yieldgainareaPOTunsus[,,"0"], 
              breaks = c(0, 200, 400, 600, 800, 1000, 1200), col = "#fcbba1", ylim = c(0, 3000),
              main = "(b) Country-level commodity prices\n", ylab = "", xlab = "")
text(x = 800, y = 2500, paste0("Spearman's rho: \n", corrAandB), cex = 2)
weights::wtd.hist(x = yieldGain_const, w = yieldgainareaPOTunsus[,,"0"], 
              breaks = c(0, 200, 400, 600, 800, 1000, 1200), col = "#fcbba1", ylim = c(0, 3000),
...
```{r AverageYield, include = TRUE, echo = FALSE}

# (Rainfed and irrigated) Yield in USD/ha with global average prices
yieldsValued   <- read.magpie(paste0(inputdatapath, "yieldsValued.mz"))

# Current cropland
rfYield_CURrf <- ifelse(dimSums(collapseNames(currentCroparea[, , "rainfed"]), dim = "crop") > 0,
                          dimSums(collapseNames(yieldsValued[, , "rainfed"]) * collapseNames(currentCroparea[, , "rainfed"]),
                                  dim = "crop") / dimSums(collapseNames(currentCroparea[, , "rainfed"]), dim = "crop"),
                        0)
irYield_CURir <- ifelse(dimSums(collapseNames(currentCroparea[, , "irrigated"]), dim = "crop") > 0, 
                          dimSums(collapseNames(yieldsValued[, , "irrigated"]) * collapseNames(currentCroparea[, , "irrigated"]),
                                  dim = "crop") / dimSums(collapseNames(currentCroparea[, , "irrigated"]), dim = "crop"),
                        0)
rfYield_CUR <- ifelse(dimSums(currentCropland, dim = 3) > 0, 
                        dimSums(collapseNames(yieldsValued[, , "rainfed"]) * dimSums(currentCroparea, dim = "irrigation"),
                                dim = "crop") / dimSums(currentCropland, dim = 3),
                      0)
irYield_CUR <- ifelse(dimSums(currentCropland, dim = 3) > 0,
                        dimSums(collapseNames(yieldsValued[, , "irrigated"]) * dimSums(currentCroparea, dim = "irrigation"),
                                dim = "crop") / dimSums(currentCropland, dim = 3),
                      0)

# Potential cropland
potentialCroparea <- cropareaShr * avlArea
rfYield_POT <- ifelse(dimSums(potentialCroparea, dim = "crop") > 0,
                        dimSums(collapseNames(yieldsValued[, , "rainfed"]) * potentialCroparea,
                                dim = "crop") / dimSums(potentialCroparea, dim = "crop"),
                      0)
irYield_POT <- ifelse(dimSums(potentialCroparea, dim = "crop") > 0,
                        dimSums(collapseNames(yieldsValued[, , "irrigated"]) * potentialCroparea,
                                dim = "crop") / dimSums(potentialCroparea, dim = "crop"),
                      0)

# Report averages
writeLines(paste0(
                  # Average rainfed yield on all current cropland 
                  "The average yield under rainfed conditions on all of current cropland is ", 
                  round(dimSums(rfYield_CUR * dimSums(currentCropland, dim = "irrigation"), dim = 1) / dimSums(dimSums(currentCropland, dim = "irrigation"), dim = 1), digits = 1),
                  " USD per hectare.", 
                  # Average rainfed yield on all current rainfed cropland 
                  "\nThe average yield under rainfed conditions on currently rainfed cropland is ", 
                  round(dimSums(rfYield_CURrf * currentCropland[, , "rainfed"], dim = 1) / dimSums(currentCropland[, , "rainfed"], dim = 1), digits = 1),
                  " USD per hectare.",
                  # Average rainfed yield on all current irrigated cropland 
                  "\nThe average yield under rainfed conditions on currently irrigated cropland is ", 
                  round(dimSums(rfYield_CURrf * currentCropland[, , "irrigated"], dim = 1) / dimSums(currentCropland[, , "irrigated"], dim = 1), digits = 1),
                  " USD per hectare.",
                  # Average irrigated yield on all current cropland 
                  "\nThe average yield under irrigated conditions on all of current cropland is ", 
                  round(dimSums(irYield_CUR * dimSums(currentCropland, dim = "irrigation"), dim = 1) / dimSums(dimSums(currentCropland, dim = "irrigation"), dim = 1), digits = 1),
                  " USD per hectare.", 
                  # Average irrigated yield on irrigated cropland
                  "\nThe average yield under irrigated conditions on currently rainfed cropland is ", 
                  round(dimSums(irYield_CURir * currentCropland[, , "rainfed"], dim = 1) / dimSums(currentCropland[, , "rainfed"], dim = 1), digits = 1),
                  " USD per hectare.",
                  # Average irrigated yield on irrigated cropland
                  "\nThe average yield under irrigated conditions on currently irrigated cropland is ", 
                  round(dimSums(irYield_CURir * currentCropland[, , "irrigated"], dim = 1) / dimSums(currentCropland[, , "irrigated"], dim = 1), digits = 1),
                  " USD per hectare."
                  ))

# Common arguments:
legendtitle  <- "USD per ha"
colNA        <- "#d9d9d9"
ylim         <- c(-6500000, 8300000)
xlim         <- c(-12577316, 15581284)

# Rainfed Yield
legendbreaksA <- c(0, 250, 500, 750, 1000, 2000, 3000, 4000, 5000, 6000)
# Transform magpie object to raster object
rfYield_POT <- mrwaterplots::toolRasterTransform(x = rfYield_POT)
rfYield_POT[rfYield_POT < min(legendbreaksA)]  <- min(legendbreaksA)
rfYield_POT[rfYield_POT >= (max(legendbreaksA)-1)]  <- max(legendbreaksA)-1

# Yield gain
legendbreaksB <- c(0, 200, 400, 600, 800, 1000, 1200)
# Transform magpie object to raster object
yieldGain_GLO <- mrwaterplots::toolRasterTransform(x = yieldGain_GLO)
yieldGain_GLO[yieldGain_GLO < min(legendbreaksB)]  <- min(legendbreaksB)
yieldGain_GLO[yieldGain_GLO >= (max(legendbreaksB)-1)]  <- max(legendbreaksB)-1

jpeg(paste0(outputdatapath, "Yields", ".jpeg"), width = 8, height = 9, units = "in", res = 400)

par(mar = c(3, 2, 2, 2), 
    mfrow = c(2, 1), 
    bg = "transparent")

### Graph 1: Rainfed Yield
plot(landMask, border = "white", col = "white", ylim = ylim, xlim = xlim, asp = NA, axes = FALSE)
plot(rfYield_POT, ylim = ylim, xlim = xlim, asp = NA, axes = FALSE,
     legend       = FALSE,
     col          = c("#fff7fb", "#ece2f0", "#d0d1e6", "#a6bddb", "#67a9cf", "#3690c0", "#02818a", "#016c59", "#014636"),
     breaks       = legendbreaksA,
     colNA        = colNA,
     add          = TRUE)
plot(landMask, border = "white", col = "white", ylim = ylim, xlim = xlim, asp = NA, axes = FALSE, add = T)
plot(worldCountries, ylim = ylim, xlim = xlim, asp = NA, axes = FALSE, add = TRUE)
title("(a) Average rainfed yield", outer = FALSE, cex.main = 2, line = 0.5)
  # Legend
  plot(rfYield_POT,
       legend.only   = TRUE,
       horizontal    = TRUE,
       ...
       breaks        = legendbreaksA,
       colNA         = colNA,
       legend.args   = list(text = legendtitle,
                           side = 3, font = 1, line = 0.5, cex = 1),
       axis.args     = list(cex.axis = 0.8, at = c(0, 1000, 2000, 3000, 4000, 5000, 6000),
...
### Graph 2: Yield gain
plot(landMask, border = "white", col = "white", ylim = ylim, xlim = xlim, asp = NA, axes = FALSE)
plot(yieldGain_GLO, ylim = ylim, xlim = xlim,
     legend       = FALSE,
     col          = c("#fcfbfd", "#bcbddc", "#807dba", "#6a51a3", "#54278f", "#3f007d"),
     breaks       = legendbreaksB,
     colNA        = "#d9d9d9",
     add          = T)
plot(landMask, border = "white", col = "white", ylim = ylim, xlim = xlim, asp = NA, axes = FALSE, add = T)
plot(worldCountries, ylim = ylim, xlim = xlim, asp = NA, axes = FALSE, add = TRUE)
title("(b) Yield gain", outer = FALSE, cex.main = 2, line = 0.5)
  # Legend
  plot(yieldGain_GLO,
       legend.only   = TRUE,
       horizontal    = TRUE,
       ...
       legend.args   = list(text = legendtitle,
                           side = 3, font = 1, line = 0.5, cex = 1),
       axis.args     = list(cex.axis = 0.8, at = legendbreaksB,
                           line = 0, tick = FALSE, hadj = 0.4, padj = -1.5),
       smallplot     = c(0.4, 0.75, 0.1, 0.15),
       add = TRUE)
