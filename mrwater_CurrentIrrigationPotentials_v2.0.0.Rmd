...
We distinguish two yield value gain thresholds (>300 USD/ha and >600 USD/ha).
...
```{r PIAcurveFunctionYieldgainarea, include=TRUE, echo=FALSE}
...
# To Do: Decide which curve plots to show

```{r PIAcurveFunctionUNSUSvsSUS, include=TRUE, echo=FALSE}
...
plotIADcurve <- function(version     = "HalfEarth",
  ...
  if (aggregation == "countries") {
    ...
    tmp    <- as.data.frame(dimSums(yieldgainareaCURunsus[, , yscale], dim = c(1.1, 1.2)))
    tmp$IrrigArea <- "Yield value gain \nareas \non current cropland \n(CUR-NOLIM)"
    df <- rbind(df, tmp)
...
for (g in gt) {
  ...
  # Actually Irrigated Land
  data <- as.data.frame(dimSums(yieldgainareaIRRunsus[,,g], dim = c(1.1, 1.2)))
  data <- data.frame(Country = data$Region, "IRR" = data$Value, GT = g)
...
for (g in c(0, 300, 600)) {
  ...
  global <- data.frame(Scenario = "IRR", 
...
The Fulfilled-Share-Map shows the share of current irrigation water demand under full irrigation requirements (IRR-NOLIM) that can be fulfilled by locally available renewable water resources captured in our data set (IRR-UNSUS). 
