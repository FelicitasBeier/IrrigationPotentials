...
plotIADcurve <- function(version     = "HalfEarth",
  ...
  if (aggregation == "countries") {
    ...
    tmp    <- as.data.frame(dimSums(potCroplandUNSUS[, , yscale], dim = c(1.1, 1.2)))
    tmp$IrrigArea <- "Unsustainable \nPotentially Irrigated Areas \non potential cropland \n(POT-UNSUS)"
    df <- rbind(df, tmp)
    ...
    tmp <- framestructure
    for (b in selectedBasins) {
      tmp$Value[tmp$Region == b] <- dimSums(potCroplandUNSUS[toolSelectRiverBasin(basinname = b), , yscale], dim = 1)
    }
    tmp$Group <- "Unsustainable \nPotentially Irrigated Areas \non potential cropland \n(POT-UNSUS)"
    df <- rbind(df, tmp)
...
plotIADcurve <- function(version     = "HalfEarth",
  ...
  if (aggregation == "countries") {
    ...
    tmp$IrrigArea <- "Areas with yield value gain through irrigation \non current cropland \n(CUR-UNSUS)"
    df <- rbind(df, tmp)
    ...
    tmp$Group <- "Areas with yield value gain through irrigation \non current cropland \n(CUR-UNSUS)"
    df <- rbind(df, tmp)
...
```{r IADcurveFunctionSUSvUNSUSvsYieldgainCUR, include=TRUE, echo=FALSE}
...
plotIADcurve <- function(version     = "HalfEarth",
  ...
  if (aggregation == "countries") {
    ...
    tmp    <- as.data.frame(dimSums(yieldgainareaCURunsus[, , yscale], dim = c(1.1, 1.2)))
    tmp$IrrigArea <- "Areas with yield value \ngain through irrigation \non current cropland \n(POT-UNSUS)"
    df <- rbind(df, tmp)
    ...
    tmp$Group <- "Areas with yield value \ngain through irrigation \non potential cropland \n(POT-UNSUS)"
    df <- rbind(df, tmp)
...
plotIADcurve(version = version, aggregation = "countries", range = c(0, 1000))

plotIADcurve(version = version, aggregation = "basins", range = c(0, 1000))
